...
class CloudExportForm(Form):
    def __init__(self, doc, selectedRevisions):
        ...
        self.Text = "Export Revision Clouds"
        self.Width = 450
        self.Height = 150
        self.StartPosition = FormStartPosition.CenterScreen
        ...
        self.buttonExport = Button(Text="Export CSV", Width=100)

        self.buttonCancel.Click += self.onCancel
        self.buttonExport.Click += self.onExport

        panelButtons.Controls.AddRange(Array[Control]([self.buttonExport, self.buttonCancel]))
        self.Controls.AddRange(Array[Control]([lblCloudFilter, self.comboCloudScope, panelButtons]))

    def filterClouds(self):
        cloudFilter = self.comboCloudScope.SelectedItem
...
def main():
    ...
    cloudForm = CloudExportForm(doc, revisionForm.selectedRevisions)
